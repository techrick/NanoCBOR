...
void nanocbor_decoder_init(nanocbor_value_t *value, const uint8_t *buf, size_t len)
{
    value->cur = buf;
    value->end = buf + len;
    value->flags = 0;
...
bool nanocbor_at_end(const nanocbor_value_t *it)
{
    ...
    if (_over_end(it) || /* Number of items exhausted */
        ...
        ((nanocbor_container_indefinite(it) && *it->cur == (NANOCBOR_TYPE_FLOAT << NANOCBOR_TYPE_OFFSET | NANOCBOR_VALUE_MASK))) ||
...
static int _get_and_advance_uint8(nanocbor_value_t *cvalue, uint8_t *value, int type)
{
    uint64_t tmp = 0;
...
static int _get_and_advance_uint16(nanocbor_value_t *cvalue, uint16_t *value, int type)
{
    uint64_t tmp = 0;
...
static int _get_and_advance_uint32(nanocbor_value_t *cvalue, uint32_t *value, int type)
{
    uint64_t tmp = 0;
...
static int _get_and_advance_uint64(nanocbor_value_t *cvalue, uint64_t *value, int type)
{
    uint64_t tmp = 0;
...
static int _get_and_advance_int64(nanocbor_value_t *cvalue, int64_t *value, uint8_t max, uint64_t bound)
{
    ...
    if (res == 1 + sizeof(uint16_t))
    {
        ...
        *ifloat = (tmp & HALF_SIGN_MASK) << (FLOAT_SIGN_POS - HALF_SIGN_POS);
        ...
        else
        {
            ...
            *ifloat |= ((exponent + HALF_EXP_TO_FLOAT) << HALF_FLOAT_EXP_POS_DIFF) | (significant << HALF_FLOAT_EXP_POS_DIFF);
        }
        return _advance_if(cvalue, res);
    }
...
static int _enter_container(const nanocbor_value_t *it, nanocbor_value_t *container, uint8_t type)
{
    ...
    {
        container->flags = NANOCBOR_DECODER_FLAG_INDEFINITE | NANOCBOR_DECODER_FLAG_CONTAINER;
...
int nanocbor_get_subcbor(nanocbor_value_t *it, const uint8_t **start, size_t *len)
{
    *start = it->cur;
...
int nanocbor_get_key_tstr(nanocbor_value_t *start, const char *key, nanocbor_value_t *value)
{
    int res = NANOCBOR_NOT_FOUND;
...
int nanocbor_get_key_uint(nanocbor_value_t *start, uint32_t key, nanocbor_value_t *value)
{
    int res = NANOCBOR_NOT_FOUND;
    uint32_t item = 0;
    ...
    *value = *start;
    ...
    while (!nanocbor_at_end(value))
    {
        ...
        if ((call_res = nanocbor_get_uint32(value, &item)) < 0)
        {
            return call_res;
        }
